...
export default function GivingStatement() {
  ...
  const yearDonations = useMemo(() => {
    return allDonations
      .filter((d) => d.person_id === id && moment(d.donation_date).year().toString() === year)
      .sort((a, b) => new Date(a.donation_date) - new Date(b.donation_date));
  }, [allDonations, id, year]);

  const total = yearDonations.reduce((s, d) => s + (d.amount || 0), 0);
  const fundName = (fid) => funds.find((f) => f.id === fid)?.name || 'Unassigned';
  const fmt = (v) => `$${v.toFixed(2)}`;
  const years = Array.from({ length: 5 }, (_, i) => (moment().year() - i).toString());

  if (loading) {
    return <div className="min-h-screen flex items-center justify-center text-slate-400 text-sm">Loading statement...</div>;
  }

  if (!person) {
    return <div className="min-h-screen flex items-center justify-center text-slate-400 text-sm">Person not found.</div>;
  }
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-2xl mx-auto bg-white shadow-sm print:shadow-none print:max-w-none my-8 print:my-0 p-10 print:p-8">
        ...
        {yearDonations.length === 0 ? (
          <p className="text-sm text-slate-500 italic py-8 text-center">No donations recorded for {year}.</p>
        ) : (
